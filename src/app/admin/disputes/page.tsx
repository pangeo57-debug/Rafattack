  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-6 space-y-4">
        {disputes.length === 0 && <p className="text-slate-500">No open disputes.</p>}
        {disputes.map((t) => (
          <div key={t.id} className={`${ui.card} p-4`}>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <Link href={`/dashboard/orders/${t.id}`} className="font-medium text-slate-900 hover:underline">
                {t.listing.title}
              </Link>
              <p className="text-sm text-slate-500">{formatDate(t.createdAt)}</p>
            </div>
